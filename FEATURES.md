...
The optional LLM receives the exact selected period, canonical totals, coverage and representative evidence. A selected call includes its current dynamic CDR, all available legs, SIP/RTP analysis, trends and related calls; selected reports are revalidated and recalculated by VoxyWatch. Identifiers are masked before provider transmission, while opaque references preserve tool correlation.

## Speech to text (Beta)

Generate searchable caller/callee transcripts on demand with a managed local `whisper.cpp` engine or an explicitly selected OpenAI provider. Export TXT/JSON/SRT and optionally hand the visible transcript to LLM chat. The feature is OFF by default, operator/admin-only and isolated from capture; unattended processing remains locked during Beta. [Speech to text Beta guide](SPEECH_TO_TEXT_BETA.md).
...
> Source of truth for the website, datasheets, sales decks and tutorial planning. Updated for **v3.68.0** (2026-08-09).
...
| **Operations: trunk, IP, fraud and Flash Call incidents** | Peer Operations submenus separate the general incident lifecycle, per-trunk health, correlated IP/IP:port failures, fraud and Flash Call findings without mixing them with settings. Fraud and Flash share a prefetched, non-blocking operational snapshot. Flash Calls use passive deterministic analysis of originator `CANCEL` timing, `487`, no-answer/no-media evidence and destination fan-out. Controls remain in Configuration. [Operational guide](FLASH_CALL_DETECTION.md). |
...
| Dashboard & CDR base | **Window-accurate KPIs** — Attempts, Answered, Active-now, ASR, NER, ACD, MOS, PDD, minutes and concurrency aggregated from continuous rollups for the *exact* range picked (hour / today / yesterday / custom / all), not a live snapshot. ~48-point trend charts (attempts, answered, concurrency, CPS, ASR/NER) + distributions (disconnect causes by SIP family, duration, PDD, codecs, MOS). INVITE/optional-CANCEL traffic with no SIP response stays searchable but is separated from commercial KPIs into an unanswered-SIP activity chart; final 3xx responses count as rejected calls. Missing-trunk discovery requires repeated IP:port evidence and suppresses high-fan-out singleton port spray. Sortable/filterable/CSV CDR base at millions of rows (keyset + trigram search). Bilingual EN/ES. Honest MOS: shows "no data" when the source lacks RTCP/RTP rather than inventing one. |
...
| Self-hosted, period | Your hardware, your data. No cloud dependency. LLM keys are yours (OpenAI/Anthropic/Google/OpenRouter/DeepSeek/Groq/Perplexity/custom) and optional. |
...
- Current version channel: see `latest.json`. All claims in this file are shipped as of v3.68.0.
